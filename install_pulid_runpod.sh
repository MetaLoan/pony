...
if [ ! -f "$PULID_MODEL" ]; then
    echo "下载 PuLID 模型..."
    huggingface-cli download ToTheBeginning/PuLID ip-adapter_pulid_sdxl_fp16.safetensors --local-dir "$COMFYUI_PATH/models/pulid"
fi
...
if [ ! -f "$EVA_CLIP" ]; then
    echo "下载 EVA-CLIP..."
    huggingface-cli download ToTheBeginning/PuLID EVA02_CLIP_L_336_psz14_s6B.pt --local-dir "$COMFYUI_PATH/models/clip"
fi
...
if [ ! -d "$INSIGHTFACE_DIR" ]; then
    ...
    unzip -q buffalo_l.zip -d "$INSIGHTFACE_DIR"
    rm buffalo_l.zip
fi
